...
We can also compare the clustering we performed with Seurat (resolution = 0.5). The `testClusters` function will test if the Seurat clustering are statistically significant and return the labels a

```{r}
# Find the optimal number of clusters given the result from Seurat
final_labels <- testClusters(mt, cluster_ids = as.character(sc_qc_GSM5102900_sct@meta.data$SCT_snn_res.0.3), 
                             alpha = 0.05)
